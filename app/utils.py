...
class Util(object):
    log = cl.customLogger(logging.INFO, "logs/general_logger.log")
    ...
    def sleep(self, sec, info=""):
        ...
        try:
            self.log.info("Waiting :: '" + str(sec) + "' seconds")
            time.sleep(sec)
    ...
    def isValidUrl(self,url, qualifying=('scheme', 'netloc')):
        tokens = urlparse(url)
        try:
            all(getattr(tokens, qualifying_attr)
               for qualifying_attr in qualifying)
            return True
        except:
            self.log.info("Invalid URL provided :: '" + str(url) )
            return False
    
    def extractDomainName(self, link: str, allowed_domains: list =["alibaba", "temu", "amazon", "aliexpress"]):
        if self.isValidUrl(link):
            raw_url = link.split("//")
            if len(raw_url) > 1:
                domain_tokens = raw_url[1].split(".")
                if len(domain_tokens) > 1:
                    for token_index in range(3):
                        domain = domain_tokens[token_index]
                        if domain in allowed_domains:
                            return domain
                if domain in allowed_domains:
                    return domain
            if raw_url in allowed_domains:
                return raw_url
